...
- `VITE_API_TOKEN` - Токен для генерации ссылок Telegram (не для авторизации пользователя)

**Важно**: 
- Токен авторизации пользователя получается через форму входа и безопасно хранится с проверкой срока действия
- `VITE_API_TOKEN` используется только для генерации ссылок для Telegram пользователей
