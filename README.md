...
**Важно**: 
- Токен авторизации получается через форму входа и безопасно хранится с проверкой срока действия
- При истечении токена или любой ошибке авторизации происходит автоматический редирект на страницу входа
- Все API запросы (включая генерацию ссылок) используют токен авторизации с Bearer префиксом
...
- **Авторизация** - вход по email/пароль, автоматический выход при истечении токена ([подробнее](./AUTHENTICATION.md))
...
- Безопасное хранение токена с проверкой срока действия
- Генерация ссылок для онбординга с fallback для HTTP
